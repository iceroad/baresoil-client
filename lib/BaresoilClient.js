...
BaresoilClient.prototype.onStateSetup_ = function() {
  ...
  onMessageFn = (function(wsMessageEvt) {
    ...
    try {
      ...
      if (verbose && BARESOIL_CLIENT_INCLUDE_VERBOSE) {
        console.error('BaresoilClient: setup: recv', JSON.stringify(inArray));
      }
      ...
      if (cmd === 'session_response') {
        if (inArray[1][0]) {
          ...
          this.error_ = inArray[1][0];
          if (verbose && BARESOIL_CLIENT_INCLUDE_VERBOSE) {
            console.error('BaresoilClient: session rejected:', this.error_);
          }
          this.transitionTo_('error');
          return;
        }
        ...
        if (inArray[1].length > 1) {
          this['sessionPacket'] = inArray[1][1];
        }
      ...
      if (cmd === 'end_connection') {
        ...
        killData = inArray[2];
        if (verbose && BARESOIL_CLIENT_INCLUDE_VERBOSE) {
          console.error('BaresoilClient: server is ending connection:', e);
        }
        this.transitionTo_('error');
        return;
      }
      ...
      // Garbled packet.
      console.error(
          'BaresoilClient: invalid packet from server during setup:',
          wsMessageEvt.data, ' exception:', e.message);
      if (verbose) {
  ...
  this.sendJson_(['session_request', this['sessionPacket'] || null]);
};
    ...
    try {
      ...
      if (cmd === 'rpc_response') {
        ...
        if (rpcCallback) {
          ...
          rpcCallback.apply(this, rpcResponse['fnResults']);
        } else {
          if (verbose && BARESOIL_CLIENT_INCLUDE_VERBOSE) {
            console.warn(
