 ...
 * @property {*} endpoint Server's Websocket endpoint URL.
...
function BaresoilClient(options) {
  ...
  if (!options['endpoint']) {
    if(!options['server']) {
      throw new Error('"options.server" must be specified if "options.endpoint" is not.');
    }
    if(!options['server']) {
      throw new Error('"options.server" must be specified if "options.endpoint" is not.');
    }
    options['endpoint'] = options['server'].replace(/\/+$/g, '') + '/__bs__/live';
  }
  if (typeof options['endpoint'] != 'string' || (
        !options['endpoint'].match(/^https{0,1}:\/\//i) &&
        !options['endpoint'].match(/^wss{0,1}:\/\//i))) {
    throw new Error('Invalid "endpoint" parameter: ' + options['endpoint']);
  }
...
BaresoilClient.prototype.resetState_ = function() {
  ...
  this.listeners_ = [];           // WebSocket listener references
}
...
BaresoilClient.prototype.killSocket_ = function() {
  var ws = this.webSocket_;
  if (ws) {
    this.listeners_.forEach(function(listener) {
      ws.removeEventListener(listener[0], listener[1]);
    });
    try {
      ws.close();
    } catch(e) { }
...
BaresoilClient.prototype.createSocket_ = function() {
  var endpoint = this.endpoint_.replace(/^http/i, 'ws');
  var webSocket = this.webSocket_ = new WebSocket(endpoint);
...
BaresoilClient.prototype['connect'] = function() {
  ...
  return setTimeout(function() {
    ...
    listener = this.onClose_.bind(this);
    listeners.push(['close', listener]);
    webSocket.addEventListener('close', listener);

    // Listen for WebSocket open events.
    listener = this.onOpen_.bind(this);
    listeners.push(['open', listener]);
...
BaresoilClient.prototype.onOpen_ = function() {
  ...
  this.sendJson_(['session_request', {
...
BaresoilClient.prototype.onMessage_ = function(wsMessageEvt) {
  this.emit_('incoming_message_raw', wsMessageEvt.data);
  try {
  ...
  if (cmd === 'session_response') {
    ...
    if (sessionResponse['auth']) {
      this.sessionResponse_ = sessionResponse['userData'] || {};
      this.flushOutbox_();
      return this.transitionTo_('connected');
    } else {
      this.error_ = new Error(sessionResponse['error']
        ? sessionResponse['error']['message'] : 'Session refused by server.');
      return this.transitionTo_('error', this.error_);
    }
  }
...
BaresoilClient.prototype['setConfigParameter'] = function(param, value) {
  ...
  if (param === 'endpoint') return this.endpoint_ = value;
 ...
 * @public
 */
BaresoilClient.prototype['getEndpoint'] = function() {
  return this.endpoint_;
};


/**
 * Returns the current SessionRequest packet.
 *
 * @public
 */
BaresoilClient.prototype['getSessionRequest'] = function() {
  return this.sessionRequest_;
}


/**
 * Sets the current SessionRequest packet.
 *
 * @public
 */
BaresoilClient.prototype['setSessionRequest'] = function(sessionRequest) {
  this.sessionRequest_ = sessionRequest;
}

/**
 * Returns the current SessionResponse packet.
 *
 * @public
 */
BaresoilClient.prototype['getSessionResponse'] = function() {
...
BaresoilClient.prototype.sendJson_ = function(value) {
  ...
  const outStr = JSON.stringify(value);
  this.emit_('outgoing_message_raw', outStr);
  this.webSocket_.send(outStr);
};
