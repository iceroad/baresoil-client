    ...
    try {
      ...
      if (cmd === 'session_response') {
        ...
        if (inArray[1].length > 1) {
          this.sessionRequest_ = inArray[1][1];
        }
        transitionedOut = true;
      ...
      if (cmd === 'user_event') {
        var userEvent = inArray[1];
        this.emit_.call(this, userEvent.name, userEvent.data);
        return;
      }
...
if (typeof module == 'object' && typeof require == 'function') {
  // __Node.js__: the `ws` module provides the WebSocket implementation, and
